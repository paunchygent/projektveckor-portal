# Read Me First (Agents)

Use this file as the starting point when you begin a new session.

## Behavioral rules (must follow)

- **Rules-first:** read `.agents/rules/000-rule-index.md` before making structural decisions.
- **Stable agent docs:** keep headings and section order unchanged in `.agents/readme-first.md` and `.agents/handoff.md`.
- **No secrets:** never add API keys/tokens, passwords, or personal data to `.agents/`, `docs/`, or committed config.
- **Prefer canonical commands:** use `pdm run ...` for backend tasks and `pnpm -C frontend ...` for frontend tasks.

## What this repo is

Projektveckor Portal is a small teacher-first portal for publishing project-week “entrypoints” as a web page:
- hosted on `hule.education`,
- linking into SharePoint/Teams resources (PDF/DOCX/mapp-länkar),
- with a consistent look and easy navigation.

## Read order (mandatory)

1. `docs/index.md`
2. `.agents/rules/000-rule-index.md`
3. `AGENTS.md`
4. `.agents/handoff.md`

## Key commands

- Backend (dev): `pdm run dev`
- Frontend (dev): `pnpm -C frontend dev`
- Frontend (build): `pnpm -C frontend build`
- Lint: `pdm run lint`
- Tests: `pdm run test`

## Session handoff

Before ending a session, update `.agents/handoff.md` (what changed, decisions, next steps).
